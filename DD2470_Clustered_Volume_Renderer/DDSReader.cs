...
namespace DD2470_Clustered_Volume_Renderer
{
    ...
    internal class DDSReader
    {
        ...
        public static unsafe Texture LoadTexture(string path, bool generateMipmaps, bool force_srgb)
        {
            byte[] fileContents = File.ReadAllBytes(path);
            DDSImageRef imageRef = LoadImage(fileContents);
        ...
        public static unsafe Texture LoadTextureMM(string path, bool generateMipmaps, bool force_srgb)
        {
            using MemoryMappedFile file = MemoryMappedFile.CreateFromFile(path);
            using MemoryMappedViewAccessor accessor = file.CreateViewAccessor();
            byte* ptr = null;
            accessor.SafeMemoryMappedViewHandle.AcquirePointer(ref ptr);
            Span<byte> fileContents = new Span<byte>(ptr, (int)accessor.SafeMemoryMappedViewHandle.ByteLength);
            DDSImageRef imageRef = LoadImage(fileContents);
            if (imageRef.Format == DDSImageFormat.BC7_UNORM && force_srgb)
            {
                imageRef.Format = DDSImageFormat.BC7_UNORM_SRGB;
